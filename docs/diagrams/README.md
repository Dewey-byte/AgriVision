# AgriVision — Conceptual Framework Diagrams (draw.io)

Open any file in [diagrams.net](https://app.diagrams.net/) or the Draw.io desktop app: **File → Open from → Device**.

## Files

| File | Diagram(s) | Chapter section |
|------|------------|-----------------|
| [AgriVision-DFD.drawio](./AgriVision-DFD.drawio) | DFD Level 0, DFD Level 1 | g. DFD |
| [AgriVision-Use-Case-Diagram.drawio](./AgriVision-Use-Case-Diagram.drawio) | Use Case Diagram | h. Use Case |
| [AgriVision-ERD.drawio](./AgriVision-ERD.drawio) | ERD (Conceptual, clean layout) | i. ERD |
| [AgriVision-Conceptual-Framework.drawio](./AgriVision-Conceptual-Framework.drawio) | All diagrams in one file (5 tabs, includes CFD) | Full set |

## Export for thesis / Word

1. Open the `.drawio` file.
2. Select the tab (page) you need.
3. **File → Export as → PNG** (set DPI to 300) or **PDF**.
4. Insert the image into your document.

## Related documentation

- **Clean ERD (recommended):** [../ERD.md](../ERD.md)
- Narrative and Mermaid versions: [../CONCEPTUAL_FRAMEWORK.md](../CONCEPTUAL_FRAMEWORK.md)
